# Solution Walkthrough — Comparing Marketing Campaigns with Expected Utility
...
| --- | --- | --- |
| Expected value | Premium Listing Push | $2.00M |
| Expected CRRA utility (γ = 2, W = $50M) | Local Concierge Add-on | CE = $1.47M |
| Minimax regret | Premium Listing Push | Max regret = $6M |

The three rules disagree. The point of the exercise is for the learner to recognize that the disagreement is not a bug — it reflects a real tradeoff between expected profit and exposure to a -$6M downside in a Weak market environment. A risk-averse decision-maker would lean on Local Concierge Add-on; a risk-neutral one would lean on Premium Listing Push. The deliverable stops at *defending a decision rule*; the exercise does **not** ask learners to translate the comparison into a stakeholder-facing recommendation.
...
3. **Tertile cutoffs** of `revenue_proxy` land near 65 and 100. The 18 cities split exactly six per bin.
...
6. **Expected value** is the row-wise dot product of the payoff matrix with the probability vector. Premium Listing Push wins on EV ($2.00M) but has a -$6M tail.
7. **Expected CRRA utility** uses $u(x) = ((W+x)^{1-\gamma} - 1) / (1-\gamma)$. With γ = 2 and W = $50M, Local Concierge Add-on wins on certainty-equivalent profit ($1.47M vs. $1.17M for Premium). The CE-equivalent is computed by inverting the CRRA function.
8. **Minimax regret** picks Premium ($6M max regret vs. $7M for Local Concierge and $10M for Hold).
9. **Comparison and defended rule** — the learner produces a small table that names the option each of the three rules selects, then defends one rule in 1–2 sentences. Either Premium or Local Concierge is a defensible choice; what is *not* defensible is hand-waving past the disagreement. The exercise deliberately stops short of a stakeholder-facing recommendation.
10. **Sensitivity flex** to `(0.20, 0.30, 0.50)` flips Premium's EV from +$2.0M to **-$0.4M**, and Local Concierge becomes the EV-max option. The teaching takeaway: every analytical output is conditional on the probability vector, and any downstream recommendation built on top of this exercise inherits that conditionality.
...
- **Confusing "regret" with "loss".** Regret is measured against the best option in each scenario, not against zero. Hold has no losses, but it has $10M of max regret because it forgoes $10M of profit when Strong materializes.
